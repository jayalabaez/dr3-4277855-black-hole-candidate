...
Recover full 19-digit Gaia DR3 source_ids for the 47 Orbital BH
candidates in the gravitas_omniscan catalog.  The catalog stores
source_id in scientific-notation float, losing the last ~6 digits.

Strategy: use RA + Dec (which *are* stored to full precision) to
cone-search the Gaia Archive with a 1-arcsec radius and match on
G magnitude to < 0.01 mag.
...
def main():
    ...
    # Read catalog entries for Orbital BH candidates 
    entries = []
    ...
    print(f"Found {len(entries)} Orbital BH candidates to resolve")

    recovered = []
    failed = []

    if use_tap:
        # Build a single ADQL query with all positions as a UNION
        # More efficient than individual cone searches
        # Use a small cone (1 arcsec) around each position
        
        # Batch into groups of 10 to avoid query length limits
        batch_size = 10
        for batch_start in range(0, len(entries), batch_size):
            batch = entries[batch_start:batch_start + batch_size]
            
            # Build WHERE clause with OR'd cone conditions
            conditions = []
            for e in batch:
                # 1 arcsec = 1/3600 deg
                conditions.append(
            ...
            try:
                ...
                # Match each entry to the closest result by position + G mag
                for e in batch:
                    best_match = None
                    best_sep = 999.0
        ...
        # Fallback: use requests to query Gaia TAP directly
        import requests
    ...
    # Save results
    output = {
        "description": "Recovered full 19-digit Gaia DR3 source_ids via positional cross-match",
