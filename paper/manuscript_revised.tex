...
%  from an Astrometric Orbital Solution with an RGB Primary
%  MNRAS format — single-target candidate paper
%  REVISED — addressing all referee recommendations
% ====================================================================
...
The system comprises a photometric RGB primary
($T_{\rm eff} = 5922$\,K, $\log g = 2.93$;
...
We conclude that this source is a promising BH candidate warranting
urgent multi-epoch radial-velocity follow-up; independent
spectroscopic confirmation is required before any BH classification
...
(Ayala-Baez 2026, in prep.).  The system is a wide binary
($P = 424$\,d) comprising a photometric RGB primary
($M_1 = 1.34\,\Msun$) and a dark companion whose mass from
the astrometric orbit is $M_2 \simeq 12.3\,\Msun$ under Gaia
...
by the unresolved orbital motion; the RGB classification should
...
with normal RGB chromospheric emission without a hot companion.
...
constrains the 0.1--2.4\,keV X-ray luminosity to
$L_X \lesssim 5 \times 10^{30}$\,erg\,s$^{-1}$ (assuming
the RASS typical sensitivity of
$\sim$\,$10^{-13}$\,erg\,s$^{-1}$\,cm$^{-2}$ at the
source distance of 657\,pc), consistent with a non-accreting
system.
...
(Section~\ref{sec:data:simbad}), so the RGB identification
...
stripped helium star could contribute detectable flux.  We strongly
encourage such modelling in future work.
...
source is unknown.  We report posteriors both with and without
...
consistent with genuine orbital photo-centre motion.  We note
that such extreme excess-noise values can in principle arise
from poorly resolved blends or pipeline failures; the coherent
Keplerian fit and well-constrained period precision
(0.27~per~cent) argue against but do not exclude an instrumental
origin.  \emph{Independent RV confirmation is essential.}
...
$L_\star = 22\,\Lsun$ are consistent with an RGB star of
$\sim$\,$1.3\,\Msun$ with radius $R_\star = 4.5\,\Rsun$.
...
We emphasise that these posteriors propagate only the
...
would require $P_{\rm inner} \lesssim 42$\,d, yet no secondary
spectral signature or short-period photometric modulation is
detected.  \textbf{Strongly disfavoured.}
...
A black hole of $\sim$\,$12.3\,\Msun$ is the preferred
interpretation but remains a \emph{candidate} pending independent
...
values.  The lack of chemical enrichment in the RGB primary
...
\item \textbf{Multi-epoch radial-velocity monitoring}
  ...
  period, eccentricity, and semi-amplitude.  A single epoch
  would already detect the large RV variation.
...
\begin{enumerate}
\item \textbf{Orbital-only solution.}  The companion mass
...
\item \textbf{Single-temperature SED.}  Our blackbody SED check
  ...
  not substitute for a full atmospheric model fit.
\end{enumerate}
...
dormant black hole candidate} that warrants urgent multi-epoch
radial-velocity follow-up.  The large predicted RV semi-amplitude
and bright apparent magnitude make spectroscopic confirmation
straightforward.  However, because the orbital solution is purely
astrometric with no independent RV data, the companion mass remains
contingent on the Gaia pipeline assumptions, and the system must
be regarded as a \emph{candidate} until spectroscopic confirmation
is obtained.
...
all figures and numerical results.  All numerical results are in
...
$M_2$ (nominal)$^\dagger$ & 12.3 $\Msun$  & 68\% CI: [8.8, 17.5]  & This work \\
$M_2$ (inflated $\times 1.7$)$^\dagger$ & 12.3 $\Msun$ & 68\% CI: [7.1, 22.7] & This work \\
...
Primary type        & G-dwarf   & RGB       & Giant           & RGB$^a$ \\
...
system.  The RGB primary orbits the unseen companion (black circle
at the focus) with period $P = 424.4$\,d and eccentricity
...
(\textit{right}).  The dereddened photometry places the star on the
red giant branch, consistent with the GSP-Phot parameters.  These
loci are approximate; formal isochrone fitting is deferred to future
work with independent spectroscopy.}
\label{fig:cmd}
\end{figure*}
