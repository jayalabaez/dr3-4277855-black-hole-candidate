...
spectroscopic follow-up target from a systematic search of the
Gaia DR3 non-single-star (NSS) catalogue.  The source carries a
...
and (iii) the conditional mass estimate places it in an
...
In this paper we present an archival multi-wavelength
characterization and follow-up prioritization of
Gaia DR3 4277855016732107520, a candidate
...
($K_1 \approx 65\,\kms$) is among the highest for unconfirmed
\texttt{Orbital} BH candidates in the NSS catalogue, making
...
The specific contribution of this paper is fourfold: (i) we
assemble the first comprehensive multi-wavelength dossier for this
source (Gaia, 2MASS, WISE, GALEX, ROSAT, VLASS, RACS); (ii) we
propagate all identified uncertainties --- including parallax-error
inflation and stellar-model systematics --- through a full
Monte~Carlo framework to produce a conditional companion-mass
posterior; (iii) we evaluate seven non-BH scenarios in a
systematic elimination hierarchy; and (iv) we provide quantitative
follow-up predictions (RV semi-amplitude, epoch count, telescope
class) that can be used directly for proposal planning.
...
which are expected to be spurious \citep{ElBadry2024}.  We
highlight four properties that distinguish the present source as
a high-priority follow-up candidate among this population:

\begin{enumerate}
\item \textbf{Large predicted RV semi-amplitude.}  At
  $K_1 \approx 65\,\kms$, the expected signal is well above the
  typical precision floor of echelle spectrographs
  ($\sim$\,$0.1$--$1\,\kms$), enabling confirmation or refutation
  within a small number of epochs.
\item \textbf{Bright apparent magnitude.}  $G = 11.25$ places the
  source within reach of 2--4\,m class telescopes with short
  exposure times ($\lesssim 30$\,min for $R \sim 40\,000$).
\item \textbf{Both-hemisphere access.}  At Dec~$= +3.5\degr$
  (Ophiuchus), the source is observable from northern and
  southern facilities, maximising scheduling flexibility.
\item \textbf{Quantitative follow-up predictions.}  This paper
  provides a complete multi-wavelength dossier, a conditional
  mass posterior with explicit uncertainty budget, and
  specific observing recommendations (epoch count, phase
  coverage, instrument class) that can be used directly
  for proposal planning.
\end{enumerate}

\noindent
None of these properties validate the orbital solution; they
establish only that, \emph{if} the solution is genuine, the
source would be among the most efficiently confirmable BH
candidates in the current NSS catalogue.
...
\item \textbf{Follow-up feasibility.}
...
Modular analysis scripts (\texttt{scripts/02}--\texttt{09}) produce
