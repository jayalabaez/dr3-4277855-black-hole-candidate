...
quiescent system (order-of-magnitude limit
$\log L_X \lesssim 30.7$\,erg\,s$^{-1}$).
...
values.  No obvious abundance anomalies are apparent from the
broadband photometry, although detailed chemical abundance
analysis requires high-resolution spectroscopy; if confirmed,
the absence of significant enrichment would suggest minimal
...
\item ROSAT, VLASS, and RACS non-detections yield an
  order-of-magnitude X-ray upper limit
  $\log L_X \lesssim 30.7$\,erg\,s$^{-1}$, consistent with a
