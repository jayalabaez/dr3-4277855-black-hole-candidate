...
spectroscopic follow-up target, selected from the
Gaia DR3 non-single-star (NSS) catalogue on the basis of its
formally high companion mass, bright apparent magnitude, and
favourable observational properties.  The source carries a
purely astrometric orbital solution (model type \texttt{Orbital}),
which derives the companion mass entirely from the along-scan
photo-centre wobble with no independent radial-velocity orbit.
All inferred companion properties are therefore \emph{conditional
on the Gaia pipeline modelling assumptions}; follow-up studies
have shown that $\gtrsim 30$--$50$~per~cent of such
\texttt{Orbital} solutions fail independent RV validation.

The visible component has Gaia GSP-Phot parameters
$T_{\rm eff} = 5922$\,K and $\log g = 2.93$; at
RUWE~$= 9.31$ these values may be systematically biased, and
no independent spectroscopic classification exists.  We adopt a
provisional $M_1 = 1.34 \pm 0.40\,\Msun$ from isochrone matching
of the GSP-Phot parameters.
Conditional on the Gaia orbital solution, a Monte~Carlo
posterior yields a median companion mass
$M_2 = 12.3\,\Msun$ (68~per~cent CI
$[7.1,\,22.7]\,\Msun$; 90~per~cent CI
$[5.0,\,35.9]\,\Msun$) after applying a $\times 1.7$
parallax-error inflation.  This posterior is a heuristic
working estimate: the inflation factor is calibrated at the
population level, correlated parameters are sampled
independently (the full NSS covariance matrix is unavailable),
and the SED model is approximate.

Using atmosphere-corrected synthetic flux ratios, a
main-sequence star at the inferred mass would exceed the primary
flux by $\sim$\,$29$--$55\times$ in optical bands, inconsistent
with the observed single-star SED.  Of seven non-BH scenarios
evaluated conditional on the adopted solution, three are
photometrically inconsistent (MS companion, WD, NS), one is
strongly disfavoured (triple), one is not favoured (stripped He
star), and two --- astrometric artefact and chance alignment ---
\emph{cannot presently be excluded}.

The source merits priority follow-up because: (i) the
predicted RV semi-amplitude ($K_1 \approx 65\,\kms$) is large
enough for efficient ground-based confirmation with $2$--$4$\,m
class facilities; (ii) at $G = 11.25$ and
Dec~$= +3.5\degr$, it is accessible from both hemispheres;
and (iii) the conditional mass estimate places it in a regime
where, if the orbital solution is genuine,
the companion would exceed the canonical BH mass threshold
($P(M_2 > 5\,\Msun) \approx 95$~per~cent under the adopted
assumptions).  Until RV confirmation is obtained, the companion
mass and nature remain entirely provisional.
\end{abstract}
...
must be treated as a \emph{candidate} rather than a confirmed
compact object, and the exclusion of alternative scenarios is
itself conditional on the validity of the underlying
\texttt{Orbital} solution.
...
For the companion-exclusion analysis, we cross-check the
Planck-based flux ratios against atmosphere-corrected values.
Wavelength-dependent correction factors are derived from
published temperature--bolometric-correction grids
\citep[ATLAS9/MARCS and PHOENIX;][]{Castelli2003,Husser2013},
applied as multiplicative offsets to the Planck ratio in
each photometric band.  The atmosphere-corrected factors differ
from the pure Planck ratios by 2--13~per~cent
(Table~\ref{tab:fluxratios}), confirming that the MS companion
exclusion is robust at the order-of-magnitude level independent
of the detailed atmospheric model adopted.  For the stripped
helium star scenario, the atmosphere correction increases the
predicted NUV contrast from $\sim$\,$1360\times$ to
$\sim$\,$1660\times$, reinforcing the NUV-based exclusion.
...
conditional on the adopted solution.

\subsection{Mass posterior}
\label{sec:results:posterior}

\noindent\fbox{\parbox{0.95\columnwidth}{%
\textbf{Caveat.}  The posteriors below are \emph{conditional
working estimates}: they propagate published Gaia parameter
uncertainties with an approximate external parallax-error
inflation, but do \emph{not} account for systematic errors in the
\texttt{Orbital} model itself (e.g.\ an incorrect inclination or
a spurious solution).  Such errors could shift the companion mass
arbitrarily and are only detectable through independent RV
follow-up.}}
...
companion.  \emph{All verdicts in this section are conditional on the
Gaia \texttt{Orbital} solution correctly describing the system and on
the adopted primary-star model being approximately correct.}  If the
orbital solution is itself spurious
(Section~\ref{sec:gaia_vetting}), these downstream exclusions lose
their force.  Table~\ref{tab:scenarios} summarises the results,
using a four-tier verdict system: \textbf{Excluded} (physically
impossible or overwhelmingly contradicted by data, conditional on
...
\medskip\noindent
\textbf{Summary:} Conditional on the Gaia \texttt{Orbital}
solution and adopted primary-star model, three of seven
alternative scenarios are inconsistent with the photometric data
...
if the solution itself is spurious, the downstream exclusions
lose their force.  A black-hole companion is \emph{not excluded}
by the available archival data under the adopted assumptions, but
this status reflects the absence of contradicting evidence rather
than positive confirmation.  The system remains a
\emph{candidate} pending independent spectroscopic validation.
...
conditional on pipeline-derived and assumed inputs that have not
been independently verified for this source.
...
the orbital solution.  They establish only that, conditional on the
Gaia pipeline model, this source would be among the most
efficiently confirmable BH candidates in the current catalogue.
...
\item \textbf{Approximate SED model.}  Our SED check uses a
  single-temperature model with atmosphere corrections applied as
  multiplicative factors rather than a full synthetic-atmosphere fit.
  The MS companion exclusion is robust at the order-of-magnitude
  level (atmosphere corrections are 2--13~per~cent;
  Section~\ref{sec:methods:sed}), but a full PHOENIX/ATLAS9
  analysis would be preferable for characterising the primary and
  detecting faint secondary contributions.
\end{enumerate}
...
\item \textbf{Conditional companion mass.}
  ...
  inflation).  This posterior is a conditional working estimate:
  the adopted inflation factor is calibrated at the
  population level (not for this source), the independent parameter
  sampling (necessitated by the unavailability of the full NSS
  covariance matrix) may underestimate the true uncertainty, and
  the SED model is approximate.  It should not be interpreted as
  a rigorous Bayesian posterior.

\item \textbf{Companion-light exclusion (conditional).}
  Atmosphere-corrected band-by-band flux ratios show that a
  main-sequence star of the required mass would exceed the
  primary flux by $29$--$55\times$ in optical bands.  This
  exclusion is robust at the order-of-magnitude level but rests
  on the adopted primary radius, which is itself uncertain
  (Section~\ref{sec:results:stellar}).
...
Modular analysis scripts (\texttt{scripts/02}--\texttt{10}) produce
all figures and numerical results, including the population-level
...
quality flag.  The target of this paper is highlighted in bold.
...
$\sigma_{\rm NSS}$ & Tier & Hem.\ \\
 & & (mag) & (d) & & ($\Msun$) & (km\,s$^{-1}$) & & & \\
\midrule
\textbf{1} & \textbf{4277855\ldots} & \textbf{11.25} & \textbf{424} & \textbf{0.34} & \textbf{12.3} & \textbf{65.0} & \textbf{75.4} & \textbf{Gold} & \textbf{Both} \\
2 & 6281177\ldots & 11.26 & 154 & 0.18 & 5.3 & 62.7 & 24.3 & Gold & Both \\
3 & 4507516\ldots & 8.89 & 813 & 0.51 & 18.2 & 48.1 & 20.3 & Gold & Both \\
4 & 6042980\ldots & 11.96 & 678 & 0.22 & 9.1 & 47.8 & 76.3 & Gold & Both \\
5 & 6033351\ldots & 11.75 & 857 & 0.41 & 6.9 & 41.7 & 55.5 & Gold & Both \\
6 & 3153895\ldots & 9.34 & 904 & 0.00 & 16.6 & 41.7 & 18.1 & Silver & Both \\
7 & 245948\ldots  & 11.80 & 549 & 0.07 & 5.0 & 35.1 & 48.4 & Gold & N \\
8 & 6656986\ldots & 6.47 & 444 & 0.58 & 5.5 & 31.6 & 28.0 & Gold & S \\
9 & 2946055\ldots & 5.27 & 755 & 0.32 & 11.0 & 30.6 & 14.6 & Silver & Both \\
10 & 6886105\ldots & 9.46 & 768 & 0.48 & 6.9 & 30.0 & 34.9 & Gold & Both \\
11 & 4113903\ldots & 6.81 & 1233 & 0.19 & 7.3 & 25.9 & 6.1 & Bronze & Both \\
12 & 3067772\ldots & 11.64 & 984 & 0.17 & 5.7 & 25.3 & 20.8 & Silver & Both \\
13 & 3972084\ldots & 8.80 & 876 & 0.15 & 6.1 & 25.0 & 21.0 & Gold & Both \\
14 & 979031\ldots  & 9.94 & 877 & 0.22 & 5.1 & 22.5 & 18.4 & Bronze & N \\
15 & 470361\ldots  & 7.18 & 999 & 0.52 & 6.5 & 21.7 & 11.2 & Silver & N \\
16 & 5103956\ldots & 6.88 & 763 & 0.27 & 5.0 & 20.8 & 22.5 & Gold & Both \\
\bottomrule
\end{tabular}
\end{table*}
